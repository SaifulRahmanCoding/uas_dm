...
function vlookup($col, $row, $label)
{
    ...
    // kondisi data
    $data = mysqli_num_rows(mysqli_query($db, "SELECT $col,play FROM tb_training WHERE $col = '$row' AND play = '$label'"));
...
function probabilitas_play($label)
{
    ...
    // kondisi data
    $data = mysqli_num_rows(mysqli_query($db, "SELECT play FROM tb_training WHERE play = '$label'"));
...
<div id="tabelPrediksi" class="table-responsive mb-2">
    ...
    <table class="table table-light table-bordered table-sm">
        ...
        <tbody>
            ...
            // probabilitas lain
            $select_test = mysqli_query($db, "SELECT * FROM tb_testing");
            $i = 1;
